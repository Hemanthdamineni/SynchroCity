...
public class ImageSender : MonoBehaviour
{
    ...
    public string[] vehiclePartKeywords = new string[] { "Body"};
    public string[] excludeKeywords = new string[] { "Segment", "Road", "Lane", "Ground", "Terrain", "Sky", "Light", "Pine", "Maple"};
    public float minVehicleSize = 30f; // Minimum bounding box size in pixels
    public int minPartsForVehicle = 1; // Minimum parts to consider it a vehicle
            ...
            foreach (var kvp in vehicleGroups)
            {
                ...
                foreach (Vector3 point in boundingPoints)
                {
                    ...
                    if (screenPoint.z > 0)
                    {
                        ...
                        // Keep Unity's screen coordinates as-is for now
                ...
                // Calculate center in Unity screen coordinates (Y=0 at bottom)
                float centerXPixels = (minX + maxX) / 2f;
                float centerYPixels = (minY + maxY) / 2f;
                
                // CRITICAL FIX: Flip Y to match image format (Y=0 at top)
                // Unity's screen has Y=0 at bottom, but saved PNG/JPG has Y=0 at top
                centerYPixels = captureHeight - centerYPixels;
                
                // Now normalize to YOLO format (0-1)
                float centerX = centerXPixels / captureWidth;
                float centerY = centerYPixels / captureHeight;
                ...
                normWidth = Mathf.Clamp(normWidth, 0.001f, 1.0f);
                normHeight = Mathf.Clamp(normHeight, 0.001f, 1.0f);
                ...
                // DEBUG: Log bounding box details with flipped Y
                Debug.Log($"[BBOX] Vehicle: {vehicleName} | Unity Screen: ({minX:F1}, {minY:F1})-({maxX:F1}, {maxY:F1}) | " +
                         $"Flipped Center Y: {centerYPixels:F1} | YOLO: cx={centerX:F3}, cy={centerY:F3}, w={normWidth:F3}, h={normHeight:F3}");

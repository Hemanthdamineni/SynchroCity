...
class TCPImageReceiver:
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if (
                        data_size_big > 10000000 and data_size_little <= 10000000
                    ):  # 10MB limit
                    ...
                    if data_size <= 0 or data_size > 10000000:  # 10MB limit
                        ...
                        print(
                            "This might indicate a byte order issue or corrupted data"
                        )
                        break
    ...
    def process_message(self, message, client_address):
        ...
        try:
            ...
            print(
                f"Camera ID: {camera_id}, Base64 data length: {len(base64_data)}, Objects: {len(ground_truth_objects)}"
            )
            ...
            print(
                f"Image received: {len(image_bytes)} bytes from camera '{camera_id}' at {client_address[0]} with {len(ground_truth_objects)} objects"
            )
            ...
            cameras.append(
                {
                    ...
                    "object_count": len(
                        latest_images[camera_id].get("ground_truth", [])
                    ),
                }
            )
        return cameras
...
def get_frame(camera_name):
    ...
    if camera_data and "image_data" in camera_data:
        try:
            ...
            return jsonify(
                {"error": f"Failed to process image: {str(e)}", "status": "error"}
            ), 500
    else:
        return jsonify(
            {"error": "No image available for this camera", "status": "offline"}
        ), 404
